    ...
    @foreach ($invoices as $invoice)
        <div class="container page-break">
            <div class="information">
                <div class="topinfo">
                    <div class="inf">
                        <p class="toptext font_rudawbold">موختاری گەڕەکی<span>( {{ $invoice->name }} )</span></p>
                        <div class="bottomtext font_Rabar_028">
                            <p>کۆڵانی : <span>{{ convertToArabicNumerals($invoice->neighborhood) }}</span> </p>
                            <p>خانوی : <span>{{ convertToArabicNumerals($invoice->house) }}</span> </p>
                            <p>شەقام : <span>{{ convertToArabicNumerals($invoice->street) }}</span> </p>
                        </div>
                        <div>
                            <p style="margin-top: 10; " class="font_Rabar_028">قاتی :<span> {{ $invoice->floor }}</span>
                            </p>
                        </div>
                    </div>
                    <div class="photos">
                ...
                <div class="bottominfo font_Rabar_028">
                    <p>ڕەچەڵەک : <span>{{ $invoice->origin }}</span> </p>
                    <p>بەروار : <span> {{ formatAndConvertDate($invoice->date) }} </span> </p>
                    <p>کۆدی کۆمپیوتەر : <span>( {{ convertToArabicNumerals($invoice->computer_code) }} )</span> </p>
                </div>
                ...
                <table class="table table-bordered border-black">
                    ...
                    <tbody class="font_Rabar_028" style="font-size: 22px">
                        @foreach ($invoice->families as $index => $member)
                            <tr style="font-size: 22px">
                                ...
                                <td>{{ convertToArabicNumerals($member->security_code) }}</td>
                                <td>{{ convertToArabicNumerals($member->phone) }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
                <div class="d-flex font_Rabar_028"
                    ...
                    <div id="additionalinforight">
                        <p>ژ.پێناس : <span>{{ convertToArabicNumerals($invoice->identification_number) }}</span> </p>
                        <p>ڕۆژی لەدایک بوون : <span>{{ formatAndConvertDate($invoice->birthday) }}</span> </p>
                        <p>فەرمانگە : <span>{{ $invoice->office }}</span> </p>
                        <p>تۆمار : <span>{{ $invoice->record }}</span> </p>
                        <p>لاپەڕە : <span>{{ convertToArabicNumerals($invoice->page) }}</span> </p>
                    </div>
                    <div id="additionalinfo">
                        <p> ناوی دایکی سەرۆک خێزان <span style="color: red;">( {{ $invoice->mother }} )</span> </p>
                        <p>ژمارەی فۆڕمی خۆراک<span style="color: red;">(
                                {{ convertToArabicNumerals($invoice->food_form_number) }} )</span> </p>
                        <p>تێبینی: <br>&nbsp; <span style="line-height:3;">{{ $invoice->notice }}</span></p>
                    </div>
                </div>
            </div>
        </div>
